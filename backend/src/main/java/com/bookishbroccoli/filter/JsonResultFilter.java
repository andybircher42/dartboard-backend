...
/**
 * Applies a list of {@link FilterRule}s to filter items in a JSON array. Stateless — safe to reuse
 * across requests.
 *
 * <p>Provides two modes:
 *
 * <ul>
 *   <li>{@link #filterArray} — filters a bare JSON array
 *   <li>{@link #filter} — filters items inside a wrapper object (e.g. {@code
 *       {"count":3,"items":[...]}}), updating the count
 * </ul>
 */
public class JsonResultFilter {
  ...
  /**
   * Creates a filter using the given ObjectMapper for creating result arrays.
   *
   * @param objectMapper the ObjectMapper used to create new JSON array and object nodes
   */
  ...
  /**
   * Filters items inside a wrapper object. Reads the array from {@code itemsField}, applies rules,
   * and returns a copy of the wrapper with the filtered array and an updated "count" field.
   *
