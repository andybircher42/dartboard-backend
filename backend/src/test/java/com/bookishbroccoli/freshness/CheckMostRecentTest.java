...
class CheckMostRecentTest {
	...
	void freshestIsFresh() {
		...
		Optional<FreshnessResult> result = checker.checkMostRecent(List.of(old, recent), ts -> ts, policy);
	...
	void freshestIsStale() {
		...
		Optional<FreshnessResult> result = checker.checkMostRecent(List.of(old, lessOld), ts -> ts, policy);
