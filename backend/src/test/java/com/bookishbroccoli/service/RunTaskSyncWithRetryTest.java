...
class RunTaskSyncWithRetryTest extends ApifyServiceTestBase {
  ...
  void successOnFirstAttempt() throws Exception {
    ...
    wireMock.verify(
        1, postRequestedFor(urlPathEqualTo("/actor-tasks/my~task/run-sync-get-dataset-items")));
  }
  ...
  void retriesOnRetryableError_thenSucceeds() throws Exception {
    ...
    wireMock.verify(
        2, postRequestedFor(urlPathEqualTo("/actor-tasks/my~task/run-sync-get-dataset-items")));
  }
  ...
  void nonRetryableError_failsImmediately() {
    ...
    assertThrows(
        ApifyNonRetryableException.class, () -> service.runTaskSyncWithRetry("my~task", Map.of()));
  ...
  void allRetriesExhausted_throwsRuntimeException() {
    ...
    RuntimeException ex =
        assertThrows(
            RuntimeException.class, () -> service.runTaskSyncWithRetry("my~task", Map.of()));

    assertTrue(ex.getMessage().contains("attempts exhausted"));
    assertInstanceOf(ApifyRetryableException.class, ex.getCause());
    // 4 total attempts
    wireMock.verify(
        4, postRequestedFor(urlPathEqualTo("/actor-tasks/my~task/run-sync-get-dataset-items")));
  }
}
