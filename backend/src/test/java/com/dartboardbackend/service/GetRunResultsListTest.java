...
class GetRunResultsListTest extends ApifyServiceTestBase {
  ...
  void emptyArray_returnsEmptyList() throws Exception {
    wireMock.stubFor(
        get(urlPathEqualTo("/actor-runs/run123/dataset/items")).willReturn(okJson("[]")));
  ...
  void errorInItem_throwsNonRetryable() {
    ...
    assertThrows(ApifyNonRetryableException.class, () -> service.getRunResultsList("run123"));
  }
  ...
  void httpError_throwsAppropriateException() {
    ...
    assertThrows(ApifyRetryableException.class, () -> service.getRunResultsList("run123"));
  }
}
